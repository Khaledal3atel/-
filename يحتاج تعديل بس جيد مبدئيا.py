...
BOT_TOKEN = "8494424963:AAFmil6Qy-QTtsktZIx8U2r6f9fAok3NM04"
OWNER_ID = 5562144078
...
def calculate_typing_speed(base_wpm, sentence_type=None):
    ...
    return max(90, min(180, final_wpm))
...
async def speed_bot_type_sentence(context, cid, sentence, wpm, sentence_type, start_time):
    try:
        ...
        await asyncio.sleep(total_time_needed)

        elapsed_time = time.time() - start_time
        word_count = count_words_for_wpm(sentence)
        elapsed_time = max(elapsed_time, 0.01)
        actual_wpm = (word_count / elapsed_time) * 60

        final_text = (
            f"سبيد جاب\n"
            f"({actual_wpm:.2f} WPM)\n"
            f"({int(elapsed_time)} s)\n\n"
            f"لو جبت اسرع انت فحل!"
        )
        
        try:
            await context.bot.send_message(chat_id=cid, text=final_text)
        except Exception as e:
            print(f"Error sending speed bot result: {e}")
            ...
            try:
                ...
                if len(parts) == 2:
                    ...
                    if 120 <= wpm <= 210:
                        ...
                        await u.message.reply_text("السرعة يجب أن تكون بين 120 و 210")
                else:
                    await u.message.reply_text("الاستخدام: سبيد [رقم بين 120-210]")
            except ValueError:
