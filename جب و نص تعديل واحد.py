...
JAB_WORDS = ["حي", "اسرع", "هيا", "عز", "رجل", "اكتب", "سريع", "ولد", "لا", "روح", "خصم", "جلد", "اقوى", "شعر", "غزل", "قرد", "خلك", "تكلج", "شيل", "مقال", "كتابة", "رتب", "عصر", "يوم", "في", "حق", "طير", "فلك", "سفن", "متى", "تصير", "قروب", "روم", "خشب", "حد", "روق", "شفه", "فرس", "نهر", "عذب", "يجري", "برق", "حيك", "راس", "مال", "صرير", "حبيب", "براون", "نرجس", "علوم", "خرج", "استمر", "نوم", "نسوخي", "مايك", "نجس", "من", "شلون", "ليه", "مر", "كاس", "عالم", "وفر", "لي"]
...
managers = {
    ...
    "نص": NassContentManager(NASS_DRIVE_URLS, min_words=5, max_words=35)
}
...
async def cmd_leaderboard(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    types = ["جمم", "ويكي", "صج", "شك", "جش", "قص", "كرر", "شرط", "فكك", "دبل", "تر", "عكس", "فر", "E", "رق", "حر"]
    sections = []
...
async def send_auto_sentence(c: ContextTypes.DEFAULT_TYPE, cid, auto_data):
    ...
    if selected_section in ["جمم", "ويكي", "صج", "شك", "جش", "فر", "E", "نص", "قص"]:
        ...
        sent = generate_random_sentence(auto_data["uid"], JAB_WORDS, 7, 20, "جب")
        storage.save_session(auto_data["uid"], cid, f"تلقائي_{selected_section}", sent, time.time(), sent=True)
        display = format_display(sent)
        await c.bot.send_message(chat_id=cid, text=display, message_thread_id=message_thread_id)
...
async def handle_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
        ...
        if message_thread_id is None or auto_mode.get("message_thread_id") == message_thread_id:
            ...
            if auto_mode["collecting"]:
                ...
                for section, session in auto_sessions.items():
                    ...
                    if section_type in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص"]:
    ...
    if text.startswith("ريست "):
        ...
        if section in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص", "شرط", "فكك", "دبل", "تر", "عكس", "فر", "E"]:
    ...
    if command in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص"] or text in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص"]:
    ...
    for session in active_sessions:
        ...
        try:
            if typ in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص"]:
                if match_text(orig, text, "arabic"):
                    matched = True
            elif typ == "فر":
                if match_text(orig, text, "persian"):
