...
BOT_TOKEN = "8188815898:AAHq0KMbBEgVlrY50I3dAV4omxaaVrS2ds4"
OWNER_ID = 5562144078
...
def calculate_typing_speed(sentence_type=None):
    base_wpm = random.uniform(110, 195)
    
    multiplier = 1.0
    if sentence_type in ["دبل", "تر"]:
        multiplier = 1.30
    
    final_wpm = base_wpm * multiplier
...
def expand_repeat_pattern(sentence):
    matches = re.findall(r'(\S+)\((\d+)\)', sentence)
    if not matches:
        return sentence
...
async def speed_bot_type_sentence(context, cid, sentence, wpm, sentence_type, start_time):
    try:
        if sentence_type == "كرر":
            sentence = expand_repeat_pattern(sentence)
        
        words = sentence.split()
        
        if sentence_type == "دبل":
            typed_words = []
            for word in words:
        ...
        for i, word in enumerate(words):
            ...
            delay = (len(word) / chars_per_second) * random.uniform(0.9, 1.1)
            ...
            try:
                if message is None:
                    message = await context.bot.send_message(chat_id=cid, text=current_text)
                else:
...
speed_bot_last_activity = {}
speed_bot_stop_requests = defaultdict(float)

async def check_speed_bot_inactivity(context, cid):
    await asyncio.sleep(60)
    
    last_activity = speed_bot_last_activity.get(str(cid), 0)
    if time.time() - last_activity >= 60:
        if str(cid) in speed_bot_tasks:
            old_task = speed_bot_tasks[str(cid)]
            if not old_task.done():
                old_task.cancel()
            speed_bot_tasks.pop(str(cid), None)
        ...
        if speed_config["enabled"]:
            ...
            try:
                await context.bot.send_message(
                    chat_id=cid,
                    text="سبيد توقف بسبب عدم النشاط لمدة دقيقة"
                )
            except:
                pass
...
async def trigger_speed_bot_if_enabled(context, cid, sentence, sentence_type):
    try:
        ...
        speed_bot_last_activity[str(cid)] = time.time()
        ...
        wpm = calculate_typing_speed(sentence_type)
        start_time = time.time()
        
        task = asyncio.create_task(
            speed_bot_type_sentence(context, cid, sentence, wpm, sentence_type, start_time)
        )
        speed_bot_tasks[str(cid)] = task
        
        asyncio.create_task(check_speed_bot_inactivity(context, cid))
...
class Storage:
    ...
    def set_speed_bot_enabled(self, cid, enabled):
        ...
        if key not in self.data["speed_bot"]:
            self.data["speed_bot"][key] = {"enabled": enabled, "base_wpm": 130}
        else:
...
async def cmd_start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "- (سبيد) - تفعيل بوت سبيد المنافس (يتسابق معك بسرعة عشوائية)\n"
...
async def show_bot_commands(u: Update, c: ContextTypes.DEFAULT_TYPE, is_callback=False):
    ...
    msg = (
        ...
        "- (تلقائي) - وضع الجمل التلقائية المتتالية\n"
        "- (سبيد) - تفعيل بوت سبيد (يتسابق معك بسرعة عشوائية)\n\n"
        "ملاحظة: سبيد بوت منافس ذكي يتسابق معك في الكتابة\n"
        "اكتب (سبيد) مرة ثانية لإيقافه\n\n"
...
async def handle_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if u.message.reply_to_message and (storage.is_main_owner(uid) or storage.is_owner(uid) or storage.is_admin(uid)):
        ...
        if text in auto_reply_commands and text == "باند":
    ...
    if text == "سبيد":
        ...
        last_request_time = speed_bot_stop_requests.get(str(cid), 0)
        
        if current_time - last_request_time < 3:
            if speed_config["enabled"]:
                storage.set_speed_bot_enabled(cid, False)
                speed_bot_stop_requests[str(cid)] = 0
                if str(cid) in speed_bot_tasks:
                    old_task = speed_bot_tasks[str(cid)]
                    if not old_task.done():
                        old_task.cancel()
                    speed_bot_tasks.pop(str(cid), None)
                await u.message.reply_text("يرجال كسر راسك رح رح تدرب بعدين تعال العب معه واكسر راسه في كل مرة")
            else:
                speed_bot_stop_requests[str(cid)] = 0
                await u.message.reply_text("سبيد مو مفعل اصلاً")
        else:
            speed_bot_stop_requests[str(cid)] = current_time
            if speed_config["enabled"]:
                await u.message.reply_text("سبيد الحين بيكسر راسك تجهز للطحن اذا ما قدرت عليه اكتب سبيد مرة ثانيه عشان يوقف")
            else:
    ...
    if command == "رق" or text == "رق":
        async with chat_locks[cid]:
            ...
            if word_count and 1 <= word_count <= 60:
                ...
                pref_count = storage.get_preference(uid, "رق")
                if pref_count and 1 <= pref_count <= 60:
    ...
    if command == "كرر" or text == "كرر":
        async with chat_locks[cid]:
            ...
            if word_count and 4 <= word_count <= 50:
                pattern = gen_pattern_with_word_count(uid, word_count)
                if pattern:
                    ...
                    await u.message.reply_text("الرجاء اختيار رقم بين 4 و 50")
            else:
    ...
    if best_match:
        ...
        elapsed = best_elapsed if best_elapsed is not None else 0.01
